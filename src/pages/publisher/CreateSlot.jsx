...
import { createSlot } from '../../services/publisher.service';
...
export default function CreateSlot() {
  ...
  const [form, setForm] = useState({
    name: '', description: '', size: '', position: '', pricingModel: '', price: '',
    allowedCategories: [], blockedCategories: [], isActive: true,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createSlot({ ...form, price: parseFloat(form.price) });
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Create Ad Slot</h1>
      <div className="max-w-2xl bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="isActive" className="text-sm font-medium text-gray-700">Active (visible to advertisers)</label>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors">
              {loading ? <LoadingSpinner size="sm" /> : 'Create Slot'}
            </button>
