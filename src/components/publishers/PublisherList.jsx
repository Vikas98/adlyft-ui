...
export default function PublisherList({ publishers = [], onSelect, selectedId, compact }) {
  ...
  return (
    <div className={`grid gap-4 ${compact ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3'}`}>
      {publishers.map((p) => (
        <PublisherCard key={p._id} publisher={p} onSelect={onSelect} selected={selectedId === p._id} compact={compact} />
      ))}
    </div>
  );
}
